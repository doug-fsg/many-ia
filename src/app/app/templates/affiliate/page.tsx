...
export default async function TemplateAffiliateProgram() {
  ...
  try {
    if (affiliateData?.stripeConnectAccountId) {
      stripeAccount = await stripe.accounts.retrieve(affiliateData.stripeConnectAccountId)
    }
  } catch (error: any) {
    console.error('Erro ao recuperar conta do Stripe:', error.message)
  ...
  if (needsStripeSetup && affiliateData?.stripeConnectAccountId) {
    try {
      ...
      console.error('Erro ao criar link do Stripe:', error.message)
      stripeError = stripeError || error.message
    }
  }
  ...
  if (affiliateData?.stripeConnectAccountId && !stripeError) {
    try {
      ...
      stripeError = stripeError || e.message
    }
  }
            ...
            <Card>
              ...
              <CardContent>
                <div className="space-y-4">
                  <p>
                    É necessário configurar corretamente as credenciais do Stripe para utilizar o programa de afiliados.
                    Entre em contato com o suporte para obter ajuda.
                  </p>
                  <p className="text-sm text-muted-foreground">
                    Erro: {stripeError.includes("does not have access to account") 
                      ? "A chave de API não tem acesso à conta do Stripe Connect." 
                      : stripeError}
                  </p>
                  {!process.env.STRIPE_SECRET_KEY && (
                    <p className="text-sm text-red-500">
                      A variável de ambiente STRIPE_SECRET_KEY não está configurada.
                    </p>
                  )}
                </div>
              </CardContent>
              ...
              <CardContent>
                <div className="space-y-4">
                  ...
                  {stripeAccountLink?.url ? (
                    ...
                    <p className="text-sm text-muted-foreground">
                      Aguarde um momento enquanto preparamos seu link de cadastro...
                    </p>
                  )}
                </div>
              </CardContent>
